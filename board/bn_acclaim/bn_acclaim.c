...
#define MUX_DEFAULT_OMAP4() \
	...
	MV(CP(USBB1_ULPITLL_STP) , ( M7))  /* not used */ \
	...
	MV(CP(USBB1_ULPITLL_DAT4) , ( M7))  /* not used */ \
	MV(CP(USBB1_ULPITLL_DAT5) , ( M7))  /* not used */ \
	MV(CP(USBB1_ULPITLL_DAT6) , ( M7))  /* not used */ \
	MV(CP(USBB1_ULPITLL_DAT7) , ( M7))  /* not used */ \
	...
	MV(CP(UNIPRO_TX0) , ( M7_SAFE))  /* kpd_col0 configured by kernel */ \
	MV(CP(UNIPRO_TY0) , ( M7_SAFE))  /* kpd_col1 configured by kernel */ \
	MV(CP(UNIPRO_TX1) , ( M3 ))      /* gpio_173 DC_CHG_ILM */ \
